...
plant_S_mat[1,6]<-matrix(c(124))
diag(plant_S_mat) <- c(0.5441,0.7536,0.5816,0.8417,0.8349,0.9954)
plant_S_mat[cbind(2:6,1:5)] <- matrix(c(0.0103,0.1848,0.4089,0.1343,0.1466))

##high_harv[1,6] <- highHarvestFecundity 
#high_harv[6,6] <- highHarvestSurvival # Multiplier for survival rate of Adult trees
high_harv <- matrix(1, nrow = 6, ncol = 6)
high_harv[1,6] <- highHarvestFecundity    # Multiplier for fecundity rate for Adult trees
high_harv[6,6] <- highHarvestSurvival # Multiplier for survival rate of Adult trees
...
stoch_growth <- function(){
  ...
  plant_mat[2:5] <- saplingInit/4   #Setting initial population of saplings
  plant_mat[6] <- adultInit #Setting initial population of adult trees
  ...
  for (i in 1:maxt)
  {
    ...
    Nprev = sum(plant_mat)
    p <- sigmoid(plant_to_AgoutiSteepness, 50, sum(plant_mat[6]))*.1 + 0.9 # bounded between 0.9 and 1.0.... k was 0.1
    ...
    plant_animal_mat[1,6] <- sigmoid(agouti_to_PlantSteepness, agoutiCapacity/2, agouti_vec[(i+1)]) # k was 0.0025
    ...
    plant_mat_sum <- c( sum(plant_mat[1]), sum(plant_mat[2:5]), sum(plant_mat[6])) 
