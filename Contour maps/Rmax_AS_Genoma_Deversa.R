...
plant_S_mat <- matrix( 0, nrow = 7, ncol = 7)
diag(plant_S_mat) <- c(0.0628,0.2381,0.7653,0.9021,0.9618,0.9762,0.9827)
plant_S_mat[cbind(2:7,1:6)] <- matrix(c(0.0042,0.0199,0.1547,0.0829,0.0232,0.0088))
plant_S_mat[1,6:7] <- matrix( c(16.561,10.777) )
high_harv <- matrix(1, nrow = 7, ncol = 7)
high_harv[1,6:7] <- highHarvestFecundity 
high_harv[cbind(6:7,6:7)] <- highHarvestSurvival # Multiplier for survival rate of Adult trees
...
stoch_growth <- function(){
  ...
  plant_mat[5:7] <- adultInit/3  #Setting initial population of adult trees
  ...
  for (i in 1:maxt)
  {
    ...
    Nprev<- sum(plant_mat)
    p <- sigmoid(plant_to_AgoutiSteepness, 50, sum(plant_mat[5:7]))*.1 + 0.9 # bounded between 0.9 and 1.0.... k was 0.1
    ...
    plant_mat_sum <- c( sum(plant_mat[1]), sum(plant_mat[2:4]), sum(plant_mat[5:7])) 
...
#==========================================================================================================================================================
growth.array<-array(0,dim=c(length(rmaxseq),length(xseq)))
binary_growth.array<-array(0,dim=c(length(rmaxseq),length(xseq)))

agoutiGrowth<-0
num<-1 
num1<-1
high_harv[cbind(5:7,5:7)] <- highHarvestSurvival # Multiplier for survival rate of Adult trees

for(i in rmaxseq)
{
  high_harv[cbind(5:7,5:7)]<-i
  num1<-1
  for(j in xseq){
