...
for(i in xseq)
{
  p<- i   # low to high transition rate 
  num1<-1
  for(j in xseq){

    highHunting<- j # high hunting of the seed disperser, the percentage of the low hunting is unchanged 
    growth_rate <- exp(stoch_growth(p))
...
filled.contour(x = xseq, 
               ...
               ylab = "High Hunting with L/H harvest",
