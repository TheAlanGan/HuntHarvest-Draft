...
deltaD <- 0.5 # Plants' Effect on Animals
deltaP <- 0.5 # Animals' Effect on Plants

highHarvestFecundity <- 0.5 # Multiplier for fecundity rate for Adult trees under HIGH harvest
highHarvestSurvival <- 0.4 # Multiplier for survival rate of Adult trees under HIGH harvest
animalGrowth <- 0.69 # Growth rate of animals in logistic model

lowHunting <- 0.0009299442033 # Percentage of animals hunted during LOW hunting
highHunting <- 0.004181184129 # Percentage of animals hunted during HIGH hunting
...
plant_mat <- matrix(0, nrow = 14)
plant_mat[seed] <- seedlingInit/2  #Setting initial population of seedlings
plant_mat[sap] <- saplingInit/2   #Setting initial population of saplings
plant_mat[adult] <- adultInit/10 #Setting initial population of adult trees
...
plant_S_mat <- matrix( 0, nrow = 14, ncol = 14)
plant_S_mat[1,adult] <- c(0, 0.67, 9.02, 9.09, 12.43, 19.57, 19.44, 18.31, 19.77, 0)
diag(plant_S_mat) <- c(0, 0.823, 0.927, 0.975, 0.942, 0.957, 0.897, 0.876, 0.911, 0.885, 0.909, 0.889, 0.999, 0.999)
plant_S_mat[cbind(2:14,1:13)] <- c(0.039, 0.015, 0.027, 0.017, 0.048, 0.043, 0.103, 0.077, 0.089, 0.103, 0.091, 0.111, 0.001)

high_harv <- matrix(1, nrow = 14, ncol = 14)
high_harv[1,adult] <- highHarvestFecundity    # Multiplier for fecundity rate for Adult trees
high_harv[cbind(adult, adult)] <- highHarvestSurvival # Multiplier for survival rate of Adult trees
