...
seedlingCapacity <- 5000 # Carrying Capacities. Tree capacities don't matter as much. (Only adult matters)
saplingCapacity <- 1000
adultCapacity <- 300
animalCapacity <- 1
perc <- 1.0
...
stoch_growth_sobol <- function(X) # X is matrix of parameters. Columns are each parameter. Rows are different parameter sets.
{
  ...
  for (i in 1:numSamples)
  {
    ...
    highHarvestFecundity <- X[i, 3] * (1 - 0.007) + 0.007 # No mapping?
    highHarvestSurvival <- X[i, 4] * (0.914 - 0.658) + 0.658 # Mapping to [0,0.9]
    #lowHunting <- X[i, 5] * (0.25 - 0) + 0 # Mapping to [0,0.25]
    #highHunting <- X[i, 6] * (1 - 0.25) + 0.25 # Mapping to [0.25,1]
    constHunt <- X[i, 5]
    animalGrowth <- X[i, 6] * (1.1 - 0.2) + 0.2 # Mapping to [0.2,1.1]
    adultCapacity <- X[i, 7] * (600 - 50) + 50 # Mapping to [50,150]
    deltaPlant <- X[i, 8] # No need for mapping
    
    # germ <- X[i, 9] * (84 - 0.23) + 0.23
    # seedSapTrans <- X[i, 10] * (0.452 - 0.006) + 0.006
    # sapAdultTrans <- X[i, 11] * (0.4 - 0.03) + 0.03
    # seedSurv <- X[i, 12] * (0.905 - 0.3405) + 0.3405
    # sapSurv <- X[i, 13] * (0.95 - 0.61) + 0.61
    # adultSurv <- X[i, 14] * (1 - 0.644) + 0.644
    
    germ <- runif(1, 0.23, 84)
    seedSapTrans <- runif(1, 0.006, 0.452)
    sapAdultTrans <- runif(1, 0.03, 0.4)
    seedSurv <- runif(1, 0.3405, 0.905)
    sapSurv <- runif(1, 0.61, 0.95)
    adultSurv <- runif(1, 0.644, 1)
    
    
    ### Using parameters ###
    # Setting low harvest matrix
    plant_S_mat <- matrix( 0, nrow = 3, ncol = 3)
    diag(plant_S_mat) <- matrix(c(seedSurv, sapSurv, adultSurv))
    plant_S_mat[cbind(2:3,1:2)] <- c(seedSapTrans, sapAdultTrans)
    plant_S_mat[1,3] <- germ

    
    # Setting high harvest matrix
    high_harv <- matrix(1, nrow = 3, ncol = 3)    
    high_harv[1,3] <- highHarvestFecundity 
    high_harv[3,3] <- highHarvestSurvival # Multiplier for survival rate of Adult trees
    ...
    plant_mat <- matrix(0, nrow = 3)
    plant_mat[1] <- seedlingInit   # Initial populations are constants
    plant_mat[2] <- saplingInit   # So they are not parameters
    plant_mat[3] <- adultInit 
    animal_vec <- c(animalInit) * (1-delta * 0.99)
    ...
    for (j in 1:maxt)
    {
      ...
      prevN <- sum(plant_mat) # For finding stoch growth rate
      
      prevAnimal <- animal_vec[j+1]
      
      p <- sigmoid(plant_to_animalSteepness, adultCapacity/2, plant_mat[3])*delta + (1-delta)
      ...
      plant_animal_mat <- matrix(1, nrow = 3, ncol = 3)
      plant_animal_mat[1,3] <- sigmoid(animal_to_PlantSteepness, animalCapacity/2, prevAnimal)*deltaPlant + (1- deltaPlant)
...
# LHS Sobol
# lhs2 <- sobolroalhs(model = stoch_growth_sobol, factors = 8, N = 5000, p = 1, order = 1, nboot = 100)
# print(lhs2)
# plot(lhs2)


#Fourier Amplitude Sens Test (Saltelli) --- Kinda Working?
fast2 <- fast99(model = stoch_growth_sobol, factors = 8, n = 10, q.arg = list(min=0.0, max=1.0))
# print(fast2)
# plot(fast2)
