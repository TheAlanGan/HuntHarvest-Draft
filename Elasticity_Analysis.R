...
#----------------------------------------------------------------------------
#Grouped_bar_code + Example use
#----------------------------------------------------------------------------
agouti_pop <- 0
plant_animal_mat <- matrix(1, nrow = 17, ncol = 17)
plant_animal_mat[1,12:17] <- sigmoid(agouti_to_PlantSteepness, agoutiCapacity/2, agouti_pop) # k was 0.0025
plant_matrixLowHigh <- plant_mat_high
dynamic_transition_matrix <- plant_animal_mat * plant_matrixLowHigh

plants_matrix_name <- "Plant_mat_low"
if(plant_matrixLowHigh[17,17]==plant_mat_high[17,17])
{
  plants_matrix_name <- "Plant_mat_high"
}

elasticity_matrix <- sensitivity_matrix(dynamic_transition_matrix)$elas 

avg_survival_seedling_elas2 <- sum(diag(elasticity_matrix[1:4,1:4]))/4  
avg_growth_seedling_elas2 <- sum(diag(elasticity_matrix[2:5,1:4]))/4 
#Sapling
avg_survival_sapling_elas2 <- sum(diag(elasticity_matrix[5:11,5:11]))/7
avg_growth_sapling_elas2 <- sum(diag(elasticity_matrix[6:12,5:11]))/7
#Adult
avg_survival_adult_elas2 <- sum(diag(elasticity_matrix[12:17,12:17]))/6
avg_growth_adult_elas2 <- sum(diag(elasticity_matrix[13:17,12:16]))/5
avg_germination_adult_elas2 <- sum(diag(elasticity_matrix[1,12:17]))/6

#Function call (it accepts 7 arguments) -------------------------------------------

grouped_bar_code <- function(avg_survival_seedling_elas2, avg_growth_seedling_elas2,avg_survival_sapling_elas2,avg_growth_sapling_elas2,avg_survival_adult_elas2,avg_growth_adult_elas2,avg_germination_adult_elas2)
{
  library(ggplot2)
  
  # create a dataset
  stage=c(rep("Seedling" , 3) , rep("Sapling" , 3) , rep("Adult" , 3))
  demographic_process=rep(c("Survival" , "Growth" , "Germination") , 3)
  elasticity=(c(avg_survival_seedling_elas2, avg_growth_seedling_elas2,0,avg_survival_sapling_elas2,avg_growth_sapling_elas2,0,avg_survival_adult_elas2,avg_growth_adult_elas2,avg_germination_adult_elas2)) #Put the values here
  data=data.frame(stage,demographic_process,elasticity)
  
  # Grouped
  ggplot(data, aes(fill=demographic_process, y=elasticity, x=stage)) +
    geom_bar(position="dodge", stat="identity") + ggtitle(paste("Agouti: ",agouti_pop, "  -  ", plants_matrix_name)) +ylim(0,0.15)
  
}

grouped_bar_code(avg_survival_seedling_elas2, avg_growth_seedling_elas2,avg_survival_sapling_elas2,avg_growth_sapling_elas2,avg_survival_adult_elas2,avg_growth_adult_elas2,avg_germination_adult_elas2)
